...
describe('Metadata API Tests', () => {
  ...
  xtest('getAllMetadata should return at least one item from Accedo One', () => {
  ...
  test('getMetadataByKey should return a valid metadata item', () => {
    const key = 'android';
    return client.getMetadataByKey(key).then(metadata => {
  ...
  test('getMetadataByKeys should return valid metadata items', () => {
    const keys = ['android', 'colorScheme'];
    return client.getMetadataByKeys(keys).then(metadata => {
      expect(metadata).toBeTruthy();
